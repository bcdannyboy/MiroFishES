...
Usage:
    ...
    python run_twitter_simulation.py --config /path/to/simulation_config.json --run-dir /path/to/run_root --run-id run_001 --seed 17
"""
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Always use configuration from the project root .env file first:
        - LLM_API_KEY: API key
        ...
        # Fall back to the config file if .env does not define a value
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        for cfg in agent_configs:
            ...
            # Derive probability from the agent's activity level
    ...
    async def run(self, max_rounds: int = None):
        ...
        for round_num in range(total_rounds):
            ...
            # Get the agents activated for this round
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command wait mode; the environment will stay alive")
...
def setup_signal_handlers():
    ...
    This gives the program a chance to clean up resources such as the database and environment.
    """
