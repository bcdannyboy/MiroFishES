...
Dynamically pushes agent activity from simulations into the Zep graph.
"""
...
class AgentActivity:
    ...
    def _describe_like_post(self) -> str:
        """Like a post, including original text and author information."""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post, including original text and author information."""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost a post, including the original content and author."""
    ...
    def _describe_quote_post(self) -> str:
        """Quote a post, including original content, author, and quote text."""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Follow a user, including the target username."""
    ...
    def _describe_create_comment(self) -> str:
        """Create a comment, including the comment text and post context."""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_like_comment(self) -> str:
        """Like a comment, including comment text and author information."""
    ...
    def _describe_dislike_comment(self) -> str:
        """Dislike a comment, including comment text and author information."""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Watches simulation action logs and pushes new agent activity into the Zep graph in real time.
    Activities are grouped by platform and sent in batches once each platform reaches `BATCH_SIZE`.

    All meaningful actions are sent to Zep, and `action_args` carries full context such as:
    - Original text of liked or disliked posts
    - Original text of reposted or quoted posts
    - Usernames involved in follow or mute actions
    - Original text of liked or disliked comments
    """
    
    # Batch size per platform before sending.
    BATCH_SIZE = 5

    # Platform display names used in console logs.
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform. Each platform flushes after reaching `BATCH_SIZE`.
        ...
        self._skipped_count = 0     # Number of filtered-out activities (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        - FOLLOW
        - MUTE
        - LIKE_COMMENT and DISLIKE_COMMENT

        `action_args` includes the full context, such as original post text or usernames.
    ...
    def _worker_loop(self):
        """Background loop that sends activities to Zep in platform-specific batches."""
        while self._running or not self._activity_queue.empty():
            try:
                # Try to fetch an activity from the queue, with a 1-second timeout.
                try:
                    activity = self._activity_queue.get(timeout=1)

                    # Add the activity to that platform's buffer.
                    ...
                    with self._buffer_lock:
                        ...
                        # Check whether the platform has reached batch size.
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send a batch of activities to the Zep graph as a single text payload.
        ...
        # Combine multiple activities into a single newline-separated text payload.
    ...
    def _flush_remaining(self):
        ...
        # Then send any remaining buffered activities, even if the buffer is smaller than BATCH_SIZE.
        with self._buffer_lock:
...
class ZepGraphMemoryManager:
    ...
    Each simulation can maintain its own updater instance.
    """
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        ...
        """
        with cls._lock:
            # Stop the existing updater first if one already exists.
    ...
    # Prevent repeated stop_all calls.
