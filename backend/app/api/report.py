...
def generate_report():
    ...
    try:
        ...
        # Generate a report_id up front so it can be returned immediately.
        import uuid
...
def get_report_sections(report_id: str):
    ...
    The frontend can poll this endpoint to fetch completed section content
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Unlock interview only after the report is complete.
...
def get_agent_log(report_id: str):
    """
    Get the detailed execution log for Report Agent.

    Fetch each step of report generation in real time, including:
    ...
    Query params:
        from_line: Starting line number, optional, default 0, used for incremental reads
...
def stream_agent_log(report_id: str):
    """
    Get the full agent log in a single response.
...
def get_console_log(report_id: str):
    ...
    Fetch console output generated during report creation in real time, such as
    INFO and WARNING lines. Unlike the structured JSON returned by `agent-log`,
    this endpoint returns plain-text console-style logs.
...
def stream_console_log(report_id: str):
    """
    Get the full console log in a single response.
