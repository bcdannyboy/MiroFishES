...
const handleNextStep = (params = {}) => {
  ...
  if (params.maxRounds) {
    ...
    addLog('Using the auto-generated simulation rounds')
  }
 ...
 * When the user returns from Step 3 to Step 2, assume they want to exit.
 */
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // Fall back to a force stop if graceful shutdown throws
        await forceStopSimulation()
      }
    } else {
      // The environment is not running, but the process might still be alive
    ...
    // Failing to check environment status should not block the next step
...
onMounted(async () => {
  ...
  // Shut down any running simulation if the user returned from Step 3
